...
namespace InWestyGator.WebDemo.Business.Services
{
    public class PackService : IPackService
    {
        ...
        public async Task<IEnumerable<Pack>> GetPackWithHierarchyAsync(string id)
        {
            ...
            await PopulatePackHierarchy(rootPack, result);
        ...
        private async Task PopulatePackHierarchy(Pack pack, ICollection<Pack> result)
        {
            ...
            if (pack.Children != null)
            {
                ...
                {
                    // TODO: Check performance with lazy loading proxies
                    var loadedChild = await _packRepository.GetByIdAsync(child.Id);
                    await PopulatePackHierarchy(loadedChild, result);
                }
            }
        }
    }
